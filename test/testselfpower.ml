...
let result_printer = Result.( function
    | Ok s -> s
    | Error m -> m
  )

let assert_selfpower expected param =
  assert_equal ~cmp:(Result.equal String.equal String.equal ) ~printer:(result_printer) expected (Selfpower.of_string param)

let selfpower_tests =
  [
    ("with param 1" >:: fun _ctxt -> assert_selfpower Result.(return "1") "1");
    ("with param 2" >:: fun _ctxt -> assert_selfpower Result.(return  "5") "2");
    ("with param -2" >:: fun _ctxt -> assert_selfpower Result.(fail "Non null positive number only") "-2");
    ("with param -2" >:: fun _ctxt -> assert_selfpower Result.(fail "Non null positive number only") "0");
